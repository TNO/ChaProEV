...
def impact_of_departures(
    ...
    print(run_mobility_matrix.loc['truck_hub'].iloc[0:24])
    ...
    if time_slot_departures_impact > zero_threshold:
        ...
        if time_slot_departures_store > zero_threshold:
            ...
            # We need to get the split between battery spaces.
            # We split the time slot departures according to how
            # battery spaces are split in the departure location
            # We need to do this to have the right amounts
            # (as the battery spaces at the departure locations are based
            # on the impact of departures/arrivals, not on the totals)
            departing_battery_spaces_split: ty.List[float] = [
                time_slot_departures_store
                * battery_space[start_location].at[
                    time_tag, departing_battery_space
                ]
                / sum(battery_space[start_location].loc[time_tag])
                for departing_battery_space in departing_battery_spaces
            ]

            for departing_battery_space, departing_amount in zip(
                departing_battery_spaces, departing_battery_spaces_split
            ):
                ...
                if time_slot_departures > zero_threshold:
                    ...
                    if departing_amount > zero_threshold:

                        arriving_amounts.append(departing_amount)
            ...
            print(
                run_arrivals_impact.loc['truck_hub', 'truck_customer'].iloc[
                    0:24
                ]
            )
            if time_tag.hour == 14:
            ...
            print('dep')
            slot_impacts = (
                first_arrival_impact * departures,
                departures
                * (
                    0.5
                    -  2*first_arrival_impact
                    + math.sqrt(2 * first_arrival_impact)
                ),
                # departures_impact,  # Here is the issue???
                # Is it because we use the wrong time tag, or quanitty (maybe departures?)
                # Use arrivals impact from slot 2 (but mix of first and second arrivals!)
                # time_slot_departures_store
                departures
                * (
                    1
            ...
            # GETS < 0!
            # IS THIS RIght? Should we also have dep impact in first slot (if short stay?)
            Mazi = False
            print(sum(slot_impacts))

            print('USE actual group size (from probabilities) instead?')
            print(slot_impacts)
            print(time_tag)
            print(start_location, end_location)
            # print(first_arrival_impact)
            # print(0.5-2*first_arrival_impact+math.sqrt(2*first_arrival_impact))
            # exit()
            print(time_slot_departures_store)
            print(departures)
            ...
            # print('Percent', percent_in_first_slot)
            # if percent_in_first_slot > zero_threshold:
            ...
            #         input()
            #         print(slot_impacts)
            #         slot_impacts = 0.2, 0.2975, 0.0025
            #         print(slot_impacts)
            #         # exit()
            #     if time_tag.hour == 14:
            ...
            #         print(first_arrival_impact)
            #         input()
            #         slot_impacts = 0.1525, 0.295, 0.0025
            #     #     slot_impacts = define.get_slot_split(percent_in_first_slot, 0.5)
            #     #     slot_impacts = slot_impacts[0] *0.45/0.5, slot_impacts[1]*0.45/0.5, slot_impacts[2]*0.45/0.5
            # else:
            #     print('Justify')
            #     same_slot_impact: float = run_mobility_matrix.loc[
            #         start_location, end_location, time_tag
            #     ]['Departures impact']
            #     slot_impacts = (
            #         0,
            #         same_slot_impact,
            #         time_slot_departures_store - same_slot_impact,
            #     )
            #     # print(slot_impacts)
            #     # print(time_slot_departures_store)
            #     Mazi = True
            ...
            if time_tag.hour == 14:
                ...
                # exit()
                # exit()
                # print(sum(arriving_amounts) * slot_impacts[2])
                # print(arrival_time_tags)
                # AT least for 14 (but maybe for 13 and down)
                # slot 1 should be a bit bigger and alot 2 a bit smaller
                # (though it is all about slot 1, as slot 2 is a remainder)
                # exit()

            # if time_tag.hour == 12:
            #     print(
            #         run_arrivals_impact.loc[start_location, end_location].iloc[
            #             0:15
            #         ]
            #     )
            #     # SHOULD get .0025 not .025
            #     print(slot_impacts)
            #     exit()
            print('Mazinger')
            print(
            ...
            #     )
            #     print(
            #         run_mobility_matrix.loc[start_location, end_location].iloc[
            #             0:15
            #         ]
            #     )
            #     print(first_arrival_time_tag)
            #     print(slot_impacts)
            #     print(
            #         run_arrivals_impact.loc[end_location, start_location].iloc[
            #             0:15
            #         ]
            #     )
            #     print(
            #         run_mobility_matrix.loc[end_location, start_location].iloc[
            #             0:15
            #         ]
            #     )
            #     print('xkcd')
            ...
            #         ]
            #     )
            #     # Impact of arrivals seems to be 0.2025
            #     # that should be lowered a bit (to .20)
            #     # but we get .389662
            #     print('Looodhgd')
            #     print(battery_space['truck_customer'].iloc[0:11].sum(axis=1))
            ...
            #     exit()
            print(
                f'ISSUE IS MAYBe here see what is is in define?'
                f'IS it necause of travelling group size?'
                f'SHould we use 0.5?'
            )
            # if time_tag.hour == 12:
            #     exit()
            ...
            for time_slot_index in range(3):
                ...
                for space_index, (
                    ...
                    if arriving_amount > zero_threshold:
                        ...
                        #     exit()
                        print(
                            'is 0.025 (from time_slot_arrovals_impact?) but should be 0.0378378'
                        )
                        print(this_battery_space_impact_this_time)
                        # exit()
            ...
            # if Mazi:
            #     exit()
            # if arrival_time_tags[0].hour == 20:
...
def get_charging_framework(
    ...
    print(run_arrivals_impact.loc['truck_hub', 'truck_customer'].iloc[0:11])
    print('run artrivals impact')
    # exit()
...
def get_charging_profile(
    ...
    for time_tag_index, (time_tag, run_day_type) in enumerate(
        ...
        if time_tag_index == 24:
            ...
            print('Siiiiii')
            # exit()
            # location_split.to_csv('Lcoo.csv')
            # exit()
            print(
                run_arrivals_impact.loc['truck_hub', 'truck_customer'].iloc[
                    0:24
            ...
            # RUN Arrivals uipdate in wrong time slot?
            # 0.0008
            # exit()

            zooka = pd.DataFrame(index=location_split.index)
            zooka['truck_hub'] = battery_space['truck_hub'].sum(axis=1)
            zooka['truck_customer'] = battery_space['truck_customer'].sum(
                axis=1
            )
            print(zooka[0:24])
            # exit()
            print(zooka[23:42])
            # exit()
            roo = location_split - zooka
            roo[abs(roo) < zero_threshold] = 0
            print(roo.iloc[0:24])
            print(location_split.iloc[0:24])
            print('kangoo')

            exit()
            print(roo.iloc[0:24].sum())
            # print(roo.sum())
            print('kangoo')
            # laha = roo.iloc[0:24]
            # laha.to_csv('Laha.csv')
            exit()
            print(roo.iloc[24:48])
            # print(
            #     run_arrivals_impact.loc['truck_hub', 'truck_customer'].iloc[
            #         0:24
            #     ]
            ...
            print('LaST ARRIVAL IMPACT REduction is one hour too late!')
            # print(zero_threshold)
            # print(zooka)
            # print(
            #     run_mobility_matrix.loc['truck_hub', 'truck_customer'].iloc[
            #         0:16
            ...
            # )
            # print('HSK')
            print('Issue is with three departing slots instead of one?')
            print('Also first hour deps????')
            print('Negaive values in loc split and arrivals imnpact?')
            exit()
        if time_tag.hour == 0:
            print(time_tag)
        # print(time_tag_index)
